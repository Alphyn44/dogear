...
The overlay half of [dogear](https://github.com/Alphyn44/dogear). Click an element in your
running app, leave a comment on it, and have your coding agent receive that comment already
bound to the exact source file and line.
...
configured through `hosts` in `.dogear/config.json`, which is repo-wide, committed, and
documented on [`dogear-vite`](https://www.npmjs.com/package/dogear-vite)'s page. Setting it
...
Node `^20.19.0 || >=22.12.0` to build. In the browser: Firefox, Chrome and Edge. Safari is
intended and not yet verified.
