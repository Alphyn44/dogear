...
Set `DOGEAR_HOME` to move that registry off `~/.dogear` — it is the only environment
variable dogear reads, and it exists mostly so tests cannot write to your real one.
...
- **`.dogear/config.json` is deleted even if you edited it by hand.** That is the one thing
  ...
  committed — `.gitignore` deliberately does not cover it — so `git checkout` brings it
