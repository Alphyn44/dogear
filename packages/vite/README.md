...
the badge in the corner to review the batch and submit. It lands in
`.dogear/queue.json` at your git root.

Add `.dogear/queue.json` and `.dogear/*.tmp` to your `.gitignore`, or run `dogear init`
below, which does it for you. The queue is machine state and you almost certainly do not
want it committed. Note also that the plugin disables itself outside a git repository, since
that is where the queue resolves from.
...
| `transform` | `true` | Stamp `data-dogear-src` onto host JSX in dev. `false` keeps the whole overlay and falls back to locating the element by CSS selector and text snippet |
...
A bad value passed here throws, because it is your own code. The same bad value in
...
Held the modifier and nothing happened? See
[Troubleshooting](https://github.com/Alphyn44/dogear#troubleshooting).
